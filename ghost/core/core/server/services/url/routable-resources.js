...
function createFetchRoutableResources({lazyUrlService}) {
    ...
    return async function fetchRoutableResources(type, {columns = []} = {}) {
        ...
        // Post.toJSON synthesizes primary_tag when the tags relation is
        // loaded, but nothing anywhere synthesizes primary_author — filters
        // and permalinks referencing it would see thin rows and throw.
        if (options.withRelated && options.withRelated.includes('authors')) {
            for (const row of rows) {
                row.primary_author = (row.authors && row.authors[0]) || null;
            }
        }
        return rows;
    };
