...
describe('UrlServiceFacade', function () {
    ...
    describe('hasLazyBackend', function () {
        it('is false with only the eager service', function () {
            assert.equal(facade.hasLazyBackend(), false);
        });

        it('is true in lazy and in compare mode', function () {
            assert.equal(new UrlServiceFacade({urlService, lazyUrlService: {}}).hasLazyBackend(), true);
            assert.equal(new UrlServiceFacade({urlService, lazyUrlService: {}, compare: true}).hasLazyBackend(), true);
        });
    });
