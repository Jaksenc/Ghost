...
describe('Unit: services/url/routable-resources', function () {
    ...
    it('synthesizes primary_author from the loaded authors relation', async function () {
        // Post.toJSON computes primary_tag when tags are loaded; nothing
        // computes primary_author, so rows would read as thin to any
        // routes.yaml filter or permalink referencing it.
        lazyUrlService.getRequiredRelations.returns(['tags', 'authors']);
        fetchAll.resolves([
            {id: 'p1', authors: [{id: 'u1', slug: 'jane'}, {id: 'u2', slug: 'joe'}]},
            {id: 'p2', authors: []}
        ]);

        const rows = await fetchRoutableResources('posts');

        assert.deepEqual(rows[0].primary_author, {id: 'u1', slug: 'jane'});
        assert.equal(rows[1].primary_author, null);
    });

    it('leaves primary_author alone when the routing config loads no relations', async function () {
        fetchAll.resolves([{id: 'p1'}]);

        const rows = await fetchRoutableResources('posts');

        assert.ok(!('primary_author' in rows[0]));
    });
});
