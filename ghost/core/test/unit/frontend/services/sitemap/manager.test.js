        ...
        describe('build path: the index is built from routable resources on first read', function () {
            ...
            beforeEach(function () {
                ...
                urlService = {
                    ...
                    isComparing: sinon.stub().returns(false),
                    hasLazyBackend: sinon.stub().returns(true),
            ...
            it('builds once on first read in eager mode, then leaves freshness to the events', async function () {
                urlService.isLazy.returns(false);
                urlService.isComparing.returns(false);
                urlService.hasLazyBackend.returns(false);

                const siteMapManager = makeManager();
                await siteMapManager.getSiteMapXml('posts');
                sinon.assert.callCount(fetchStub, 4);

                // No invalidation with eager: the per-URL events keep the
                // index current, so site.changed must not trigger a rebuild.
                eventsToRemember['site.changed']();
                await siteMapManager.getSiteMapXml('posts');
                sinon.assert.callCount(fetchStub, 4);
            });

            it('builds in compare mode too, so the build path runs before the lazy flip', async function () {
                urlService.isLazy.returns(false);
                urlService.isComparing.returns(true);
