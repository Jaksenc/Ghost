...
// A field's type is one of the shared catalog's field types; the same enum the
// backend validates against, so admin and server never drift on the set.
export type MemberCustomFieldType = FieldType;
